...
```{r, eval=F, echo=F}
#for speed's sake, the data were downloaded once, and loaded in with the following code, rather than the code above
library(data.table)

coal <- fread('MinesProdQuarterly.txt',
              sep='|',
              stringsAsFactors=T,
              data.table=F)

mines <- fread('Mines.txt',
               sep='|',
               stringsAsFactors=T,
               data.table=F)
```
